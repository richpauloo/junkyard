MODULE UTILITIES
  ...
  PUBLIC :: UTL_ADDQUOTE, UTL_APPENDLIST, UTL_ARR2CDMATRIX,   &
  ...
  CHARACTER(LEN=10), DIMENSION(MAXDX) :: DXEXT=' '
  !
  ! ****************************************************************************
...
20    CONTINUE
 ...
 51 FORMAT(1X,200A1)
    ...
    DO IP = 1, NPE
      ...
      DO I = I1, NPE
        IF (ABS(BUFF(IP,I)).GE. 0.95) THEN
          ...
          IF(DATAEXCHANGE) &
             WRITE (IUPCC,820) PARNAM(IIPP),PARNAM(IPTR(I)),BUFF(IP,I)
          I95 = 1
        ENDIF
      ENDDO
    ENDDO
  ...
  SUBROUTINE UTL_EIGEN(IOUT,IPRC,NPE,NPS,IPTR,ITERP,PARNAM,PVALINIT,PVAL,C)
    ...
    508 FORMAT (/,12X,8A13,/,10(12X,8A13))
    ...
    !-------PRINT EIGENVALUES
    WRITE (IOUT,510)
    WRITE (IOUT,508) (PARNAM(IPTR(J)),J=1,NPE)
...
10      FORMAT(/,' *** PROGRAMMING ERROR CALLING SUBROUTINE ',a,' ***')
  ...
  SUBROUTINE UTL_SVD(IFAIL,A,AS,DTLA)
    ...
    TYPE (CDMATRIX) :: BUFF1
    TYPE (CDMATRIX) :: AI
    !DOUBLE PRECISION, DIMENSION(A%NR) :: BUFF3, E
