MODULE DEPENDENTS
  ...
  PUBLIC :: GROUP, GROUPNAM, GROUPNOBS, IGPTYPE, LLPTRDEP, LLPTRDEPGP,   &
            ...
            OBSVAL, STATISTIK, USE_FLAGDEP, USEFLAG, WTCORRELATED, WTCOS
  !
  ...
  DOUBLE PRECISION,       ALLOCATABLE, DIMENSION(:) :: WTCOS
  !
  !   PUBLIC SUBPROGRAMS
  !
...
CONTAINS
  ...
  SUBROUTINE DEP_INI_FILTERLIST(DEFGROUP,IOUT,NOBS,NEQNPTR,OBSNAM)
    ...
    NULLIFY(PTR,PIPTR)
    !   Allocate and initialize arrays related to observations
    ALLOCATE(NONDETVAL(NOBS),OBSVAL(NOBS),STATISTIK(NOBS), &
             VARIANCE(NOBS),WTCOS(NOBS),WTCORRELATED(NOBS),WTVEC(NOBS))
    NONDETVAL = 0.D0
    OBSNAM = ' '
    OBSVAL = BIGDOUBLE
    STATISTIK = BIGDOUBLE
    ...
    DEPENDENT: DO
      ...
      INFO: DO
        ...
        ELSEIF (PIPTR%KEYWORD == 'WTOSCONSTANT') THEN
          ...
          IF(WTCOSTMP .NE. 0.D0) THEN
            IF(STATFLAG .NE. 'CV') THEN
              CALL UTL_STOP &
              ('WEIGHTING BASED ON OBS&SIM VALUES REQUIRES StatFlag = CV ')
            ENDIF
          ENDIF
        ENDIF
        PIPTR => PIPTR%NEXTNODE               ! Get pointer to next entry
  ...
  SUBROUTINE DEP_INI_READ(IDEPTYPE,INUNIT,IOUT,NCOVMAT,NOBS,NDDEP,NEDEP,NTDEP)
    !   Read, store, and echo observation-related information
