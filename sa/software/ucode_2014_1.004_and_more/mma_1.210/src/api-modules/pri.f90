MODULE PRIOR_INFORMATION
  ...
  PUBLIC LLPTRPRIOR, LLPTRGPPRIOR, LLPTRPRIORFP, LLPTRGPPRIORFP, &
         MODELPRIVAL, PLOTSYMBOLPRI, PRIEQN, &
        ...
        CHECKLOG: DO J=1,NPE
          IF(XPRI(J,I) .NE. 0 .AND. LN(IPTR(J)).EQ.1) THEN
            ...
            DO II=1,MPR
              ...
              ENDIF
! FULL MATRICES MUST BE INPUT IN A MATTER CONSISTENT WITH THE PARAMETER 
...
!              ENDIF
  ...
  !-----------------------------------------------------------------------------
  SUBROUTINE PRI_INI_READ(INUNIT,IOUT,IWRITE,NPE,NCOVMAT,MPR)
