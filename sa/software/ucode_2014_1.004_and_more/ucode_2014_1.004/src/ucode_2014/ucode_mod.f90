MODULE UCODEMOD
  ...
  !   Public data
    ...
    OBSNAM, OMIT, OPT, OPTIMIZE, OPTNLUNC, &
  ...
  INTEGER, ALLOCATABLE,  DIMENSION(:)                :: SENSOMIT
  INTEGER                                            :: ISENSOMIT
  INTEGER                                            :: ALLSENSOMIT
  INTEGER                                            :: OMITTEDNP
  LOGICAL                                            :: OPT = .FALSE.
  ...
  LOGICAL                                            :: SVDPHASE1
  CHARACTER(LEN=15)                                  :: SVDUPDATE
  DOUBLE PRECISION                                   :: SVDUPDATE_PCC
  DOUBLE PRECISION                                   :: SVDUPDATE_CSSRAT
  DOUBLE PRECISION                                   :: SVDUPDATE_CSSABS
  ...
  LOGICAL                                            :: USE_WT            ! Read weight from _wt file
...
!===============================================================================
  ...
  !------------------------------------------------------------------
    ...
    !   First Set defaults
    LINEARITY = .FALSE.          ! default perform regression
    LINEARITYADV = .FALSE.          ! default perform regression
    LINADVANS = 'NO'
    LINADVCONF = .FALSE.
    LINADVPRED = .FALSE.
    ...
    SENSITIVITIES = .FALSE.     ! default perform sensitivity analysis
    ...
    SVDUPDATE_CSSRAT = 0.01D0
    SVDUPDATE_CSSABS = 1.00D0
    SVDSTARTPARSC = ' '
    SVDNEWVECTORSC = 'NO'
    ...
    USE_WT = .FALSE.            ! default not reading weight from wt file
  !
...
!      dataexchange is no longer a user option
      ...
      CALL UTL_FILTER(IERR,UCODEHEAD,IOUT,'SVDUPDATE_CSSRAT',SVDUPDATE_CSSRAT)
      CALL UTL_FILTER(IERR,UCODEHEAD,IOUT,'SVDUPDATE_CSSABS',SVDUPDATE_CSSABS)
      ...
      CALL UTL_FILTER(IERR,UCODEHEAD,IOUT,'USE_WT',USE_WT)
      ...
      ELSEIF(TMP == 'YES' .OR. TMP == 'TRUE') THEN
        ...
        IF(SVDPHASEC == 'NEW') THEN
          ...
          IF(SVDSTARTPARSC .NE. 'PARAMETER_DATA') THEN
            ...
            AMESSAGE = ' Resetting SVDstartparsc to PARAMETER_DATA'
            CALL UTL_WRITE_MESSAGE(IOUT,'no','yes','yes')
            SVDSTARTPARSC = 'PARAMETER_DATA'
          ENDIF
          IF(SVDNEWVECTORSL) THEN
        ...
        ELSEIF(SVDPHASEC == 'CONTINUE') THEN
          ...
          IF(SVDSTARTPARSC == 'PARAMETER_DATA') THEN
            ...
            CALL UTL_WRITE_MESSAGE(IOUT,'yes','yes','yes')
            AMESSAGE = &
            ' SVDstartpars is being changed to _paopt'
            CALL UTL_WRITE_MESSAGE(IOUT,'yes','yes','yes')
            SVDSTARTPARSC = '_PAOPT'
          ENDIF
          IF(SVDSTARTPARSC == '_PAOPT') THEN
        ...
        WRITE(IOUT,*)
        CALL UTL_CASE(SVDUPDATE,SVDUPDATE,1)
        IF(SVDUPDATE == 'SVDALL' .OR. SVDUPDATE == 'OPTION2_SVDSET' .OR. &
           SVDUPDATE == 'OPTION2_CSS_PCC')THEN
          CONTINUE
        ELSE
...
!   Write information (UCODE controls) to output file
    ...
    IF(SVDT) THEN
      ...
      ELSEIF(SVDUPDATE == 'OPTION2_SVDSET' .OR.  &
             SVDUPDATE == 'OPTION2_CSS_PCC') THEN
        WRITE(IOUT,100) 'SVDUPDATE = OPTION2_SVDSET or OPTION2_CSS_PCC,'
        WRITE(IOUT,100) '  so a subset of process model parameters will'
        WRITE(IOUT,100) &
        ...
        IF(SVDUPDATE == 'OPTION2_SVDSET') THEN
          WRITE(IOUT,100) 'SVDUPDATE = OPTION2_SVDSET, so'
          WRITE(IOUT,100) &
          '   SVDSET values from the parameter data block will be used '
          WRITE(IOUT,100) &
          '   to determine which parameters maintain the value assigned at the'
          WRITE(IOUT,100) &
          '   start of the SVD parameter estimation.'
        ELSE
          WRITE(IOUT,100) 'SVDUPDATE = OPTION2_CSS_PCC, so values of '
          WRITE(IOUT,100) &
          '   SVDUPDATE_PCC, SVDUPDATE_CSSRAT, and SVDUPDATE_CSSABS are used'
          WRITE(IOUT,100) &
          '   to determine which parameters maintain the value assigned at the'
          WRITE(IOUT,100) &
          '   start of the SVD parameter estimation.'
  ...
  SUBROUTINE UCODE_INI_ALLOC (IFAIL,IDEPTYPE,MPR)
    ...
    IF (NOBS>0) THEN
      ALLOCATE(CV(NOBS),OBSNAM(NOBS),MODELVAL(NOBS),OMIT(NOBS), &
               RESIDS(NOBS),SENSOMIT(NOBS),WTDRESIDS(NOBS),XSENST(NPE,NOBS))
      IF (IDEPTYPE==1) THEN
      ...
      OMIT = 0
      XSENST = BIGDOUBLE
      IF(ALLOCATED(INCLU)) INCLU = NUSEOBS
    ENDIF
    !
    IF (MPR>0) MPRAR = MPR
...
!    IF(PREDICT .AND. .NOT. SENSITIVITIES) PARAM_OMIT = .FALSE.
    ...
    DO I=1,NPS
      ...
      IF (SVDSETORIG(I) .NE. 0 .AND. SVDUPDATE == 'OPTION2_SVDSET') THEN
        SVDSETL = .TRUE.
        SVDSETI = SVDSETI + 1
...
!===============================================================================
  SUBROUTINE UCODE_INI_PARS_INSTALL (IFAIL,IOUT)
    ...
    ! formats
    10 FORMAT(/,80('<'),/)
    15 FORMAT(2X,'DERIVED PARAMETERS',/,5X,'# of DERIVED PARAMETERS = ',I0,/)
...
!===============================================================================
  SUBROUTINE UCODE_INI_SVD(IOUT,OUTNAM,MPR,NPETMP,NPSTMP,SVDLOOP,NPENOW,NPSNOW)
    ...
    230 FORMAT(//, &
        ...
        1X,' SVDUPDATE_CSSRAT ',1PE15.7,/, &
        1X,' SVDUPDATE_CSSABS ',1PE15.7,/, &
    ...
    WRITE(IOUT,242)
    WRITE(IOUT,230) SVDTC,SVDPHASEC,SVDSTARTPARSC,SVDNEWVECTORSC,SVDUPDATE, &
            SVDPRINTC,NPESVD,SVDRATIO,SVDUPDATE_PCC,SVDUPDATE_CSSRAT, &
            SVDUPDATE_CSSABS,SVD_PERTURB,SVD_MAXCHANGE,SVD_TOLPAR
    ...
    IF(SVDUPDATE == 'OPTION2_SVDSET' .OR. SVDUPDATE == 'OPTION2_CSS_PCC') THEN
      WRITE(IOUT,222)
      ...
      WRITE(IOUT,*) &
        ' *********************************************************************'
      WRITE(IOUT,300) &
        ' DETERMINING PARAMETERS TO BE SET FOR SVD_UPDATE = ',SVDUPDATE
      WRITE(IOUT,*)
      IF(SVDSETL) THEN
        WRITE(IOUT,*) &
      ...
      ELSE
        CALL UCODE_INI_SVDOPTION2 &
           (IOUT,NPE,NPNULL,OUTNAM,SVDUPDATE_PCC,SVDUPDATE_CSSRAT, &
            SVDUPDATE_CSSABS,IPTRSVDUPDATE,ICOLMAX)
      ENDIF
      WRITE(IOUT,*)(NPNULL), &
...
!===============================================================================
  SUBROUTINE UCODE_INI_SVDOPTION2(IOUT,NPE,NPNULL,OUTNAM, &
         SVDUPDATE_PCC,SVDUPDATE_CSSRAT,SVDUPDATE_CSSABS,IPTRSVDUPDATE,ICOLMAX)
    !
    ...
    DOUBLE PRECISION,                       INTENT(IN)    :: SVDUPDATE_CSSRAT
    DOUBLE PRECISION,                       INTENT(IN)    :: SVDUPDATE_CSSABS
    ...
    INTEGER                                        :: M
    INTEGER                                        :: N
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: CSSORIG
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: CSSSORT
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: CSSRATSORT
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: CSSRAT
    ...
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)  :: PCCOPT2
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)  :: PCCOPT2TMP
    DOUBLE PRECISION                               :: PCCMAX
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: PCCMAXCOL
    CHARACTER(LEN=12), ALLOCATABLE, DIMENSION(:)   :: PNORIG
    CHARACTER(LEN=12), ALLOCATABLE, DIMENSION(:)   :: PNSORT
    CHARACTER(LEN=12), ALLOCATABLE, DIMENSION(:)   :: PNTMP
    INTEGER                                        :: SMALLEST
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: HICLST
    INTEGER, ALLOCATABLE, DIMENSION(:,:)           :: HIRC
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: HICLSTORIG
    INTEGER, ALLOCATABLE, DIMENSION(:,:)           :: HIRCORIG
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)    :: HICLSTTMP
    INTEGER, ALLOCATABLE, DIMENSION(:,:)           :: HIRCTMP
    INTEGER, ALLOCATABLE, DIMENSION(:)             :: HIELIM
    INTEGER                                        :: NPEFAC
    INTEGER                                        :: ICNT
    INTEGER                                        :: NEWCNT
    INTEGER                                        :: START
    ...
    INTEGER                                        :: HC2
    INTEGER                                        :: HC3
    DOUBLE PRECISION                               :: PC1
    DOUBLE PRECISION                               :: PC2
    DOUBLE PRECISION                               :: PC3
    INTEGER, ALLOCATABLE, DIMENSION(:)             :: PARNUMSORT
    LOGICAL                                        :: AVAILABLE
    !
    20 FORMAT(/,8X,'THERE ARE NO CORRELATIONS',/, &
              10X,'NULL SPACE WILL BE FILLED BASED ON LOW SENSITIVITY',/)
    22 FORMAT(8X,'ABS(CORRELATIONS)',/, &
              7X,F8.5,5X,F8.5,5X,F8.5)
    23 FORMAT(8X,A12,1X,'HAS ALREADY BEEN IDENTIFIED TO BE SET')
    24 FORMAT('  SEARCH FOR GROUPS OF',/, &
              '  THREE INTERDEPENDENT PARAMETERS WITH CORRELATIONS >= ',F7.5)
    25 FORMAT('  NO GROUPS OF THREE PARAMETERS WITH CORRELATIONS GREATER THAN' &
              ,/,3X,1PE12.4,' WERE FOUND')
    26 FORMAT(15X,A12,1X,F8.5,6X,1PE15.6,1X,1PE15.6)
    27 FORMAT(15X,A12,17X,1PE15.6)
    28 FORMAT(8X,A12,1X,A12,1X,A12)
    29 FORMAT(8X,A12,' HAD SMALLEST CSS OF: ',1PE12.4)
    34 FORMAT(/,'  SEARCH FOR PAIRS OF',/, &
              '  INTERDEPENDENT PARAMETERS WITH CORRELATION >= ',F7.5)
    35 FORMAT('  NO PAIRS OF PARAMETERS WITH CORRELATION GREATER THAN', &
              /,3X,1PE12.4,' REMAINED AFTER EVALUATING SETS OF THREE')
    250 FORMAT(/,'  INSUFFICIENT GROUPS OF THREE TO DEFINE ALL SET PARAMETERS')
    251 FORMAT(/,'  INSUFFICIENT GROUPS OF THREE AND PAIRS TO DEFINE ALL ', &
                 'SET PARAMETERS')
    255 FORMAT(/,'  REMAINING NULL SPACE WILL BE FILLED BASED ON LOW ', &
                'SENSITIVITY',/)
    256 FORMAT(8X,A12,' HAD CSS OF: ',1PE12.4)
    334 FORMAT(/,'  SEARCH FOR PARAMETERS WITH CSS RATIO TO MAXIMUM < ',1PE12.4)
    335 FORMAT(/,'  SET THE FOLLOWING PARAMETERS: '/)
    336 FORMAT(8X,A12,' HAD CSS RATIO TO MAXIMUM CSS OF: ',1PE12.4)
    337 FORMAT(/,'  NO PARMETERS HAD CSS RATIO TO MAXIMUM CSS < ',1PE12.4)
    344 FORMAT(/,'  SEARCH FOR PARAMETERS WITH ABSOLUTE CSS < ',1PE12.4)
    345 FORMAT(/,'  SEARCH REMAINING PARAMETERS FOR ABSOLUTE CSS < ',F7.5)
    346 FORMAT(/,'  SET THE FOLLOWING PARAMETERS: '/)
    347 FORMAT(8X,A12,' HAD ABSOLUTE VALUE OF CSS OF: ',1PE12.4)
    348 FORMAT(/,'  NO PARMETERS HAD ABSOLUTE VALUE OF CSS < ',1PE12.4,/)
    349 FORMAT(/,'  NO REMAINING PARAMETERS HAD ABSOLUTE VALUE OF CSS < ', &
                                                                    1PE12.4,/)
    !
    IF(NPNULL >= NPE) THEN
      AMESSAGE = &
    ...
    ENDIF
    NPEFAC = NPE*(NPE-2)
    ALLOCATE(CSSTMP(NPE),PNTMP(NPE),CSSSORT(NPE),CSSRAT(NPE),CSSRATSORT(NPE), &
             PNSORT(NPE),CSSORIG(NPE),PNORIG(NPE))
    ALLOCATE(PCCMAT(NPE,NPE),PCCOPT2(NPE,NPE),PCCOPT2TMP(NPE,NPE), &
             PCCMAXCOL(NPE),PARNUMSORT(NPE))
    ALLOCATE(HICLST(NPEFAC),HIRC(NPEFAC,2),HIELIM(NPE))
    ALLOCATE(HICLSTORIG(NPEFAC),HIRCORIG(NPEFAC,2))
    !
    ! INITIATE
    AVAILABLE = .TRUE.
    CSSTMP = 0.D0
    ...
    HIELIM = 0
    !
    ! First set parameters with CSS ratio < SVDUPDATE_CSSRAT (default is 0.01)
    IF(NPPCC < NPNULL) THEN
      WRITE(IOUT,334)SVDUPDATE_CSSRAT
      DO I = 1,NPE
        PARNUMSORT(I) = I
      ENDDO
      CSSSORT = CSSTMP
      PNSORT = PNTMP
      CALL UTLUCODE_SHELLSORTTHREE(NPE,CSSSORT,PNSORT,PARNUMSORT)
      DO I = 1,NPE
        CSSRATSORT(I) = CSSSORT(I)/CSSSORT(NPE)
      ENDDO
      DO I = 1,NPE
        CSSRAT(I) = CSSTMP(I)/CSSSORT(NPE)
      ENDDO
      LOWRAT: DO I = 1,NPE
        IF(CSSRATSORT(I) < SVDUPDATE_CSSRAT) THEN
          IF(I == 1) WRITE(IOUT,335)
          WRITE(IOUT,*)
          WRITE(IOUT,336) PNSORT(I),CSSRATSORT(I)
          NPPCC = NPPCC + 1
          HIELIM(NPPCC) = PARNUMSORT(I)
          IF(NPPCC == NPNULL) EXIT LOWRAT
        ELSE
          IF(I == 1) WRITE(IOUT,337)SVDUPDATE_CSSRAT
          EXIT LOWRAT
        ENDIF
      ENDDO LOWRAT
    ENDIF
    !
    ! Next set parameters with absolute values of CSS  < SVDUPDATE_CSSABS (default is 1.0)
    IF(NPPCC < NPNULL) THEN
      IF(NPPCC == 0) THEN
        START = 1
        WRITE(IOUT,344)SVDUPDATE_CSSABS
      ELSE
        START = NPPCC + 1
        WRITE(IOUT,344)SVDUPDATE_CSSABS
      ENDIF
      LOWABS: DO I = START,NPE
        IF(CSSSORT(I) < SVDUPDATE_CSSABS) THEN
          IF(I == START) WRITE(IOUT,346)
          WRITE(IOUT,*)
          WRITE(IOUT,347) PNSORT(I),CSSSORT(I)
          NPPCC = NPPCC + 1
          HIELIM(NPPCC) = PARNUMSORT(I)
          IF(NPPCC == NPNULL) EXIT LOWABS
        ELSE
          IF(I == 1) WRITE(IOUT,348)SVDUPDATE_CSSABS
          IF(I == START) WRITE(IOUT,349)SVDUPDATE_CSSABS
          EXIT LOWABS
        ENDIF
      ENDDO LOWABS
    ENDIF
    ...
    IF(NPPCC < NPNULL) THEN
      ...
      DO I = 1,NPE
        PCCMAT(I,I) = -2
      ENDDO
      ! PCCMAXCOL holds max off diagonal value in each col
      PCCMAXCOL = MAXVAL(PCCMAT,DIM=1)
      PCCOPT2 = 0.D0
      ! POPULATE INTEGER ARRAY WITH CORRELATION VALUES > SVDUPDATE_PCC !
      DO I = 1,NPE
        DO J = 1,NPE
          IF(PCCMAT(I,J) >= SVDUPDATE_PCC) PCCOPT2(I,J) = PCCMAT(I,J)
        ENDDO
        PCCOPT2(I,I) = -2
      ENDDO
      !
      ...
      ICNT = 0
      NEWCNT = 0
      ! BUILD TEMP PCC ARRAY TO MODIFY AS HI COORELATIONS ARE IDENTIFIED
      DO
        ! PCCMAX holds max value in array
        PCCMAX = MAXVAL(PCCOPT2TMP)
        MLPCCOPT2 = MAXLOC(PCCOPT2TMP)
        IF(PCCMAX > 0.D0) THEN
          ICNT = ICNT + 1
          HIRC(ICNT,1) = MLPCCOPT2(1)
          HIRC(ICNT,2) = MLPCCOPT2(2)
          HICLST(ICNT) =  PCCOPT2TMP(HIRC(ICNT,1), HIRC(ICNT,2))
          PCCOPT2TMP(HIRC(ICNT,1), HIRC(ICNT,2)) = -1.D0
          PCCOPT2TMP(HIRC(ICNT,2), HIRC(ICNT,1)) = -1.D0
        ELSE
          EXIT
        ENDIF
      ENDDO
      HICLSTORIG = HICLST
      HIRCORIG = HIRC
      IF(HICLSTORIG(1) > 0.D0) THEN
      ! FIND THREESOMEs with HIGHEST CORRELATIONs AND IDENTIFY LOWEST CSS PARAM !!
        ALOOP: DO A = 1,ICNT
          IF(HICLST(A) == 0.D0) EXIT ALOOP
          DO I = 1,NPE
            IF(HIRC(A,1) == HIELIM(I) .OR. HIRC(A,2) == HIELIM(I)) CYCLE ALOOP
          ENDDO
          BLOOP: DO B = A+1,ICNT
            DO I = 1,NPE
              IF(HIRC(B,1) == HIELIM(I) .OR. HIRC(B,2) == HIELIM(I)) CYCLE BLOOP
            ENDDO
            IF(HIRC(A,1) == HIRC(B,2)) THEN
              HC1 = HIRC(A,1)
              HC2 = HIRC(B,1)
              HC3 = HIRC(A,2)
            ELSEIF(HIRC(A,1) == HIRC(B,1)) THEN
              HC1 = HIRC(A,1)
              HC2 = HIRC(B,2)
              HC3 = HIRC(A,2)
            ELSEIF(HIRC(A,2) == HIRC(B,1)) THEN
              HC1 = HIRC(A,2)
              HC2 = HIRC(B,2)
              HC3 = HIRC(A,1)
            ELSEIF(HIRC(A,2) == HIRC(B,2)) THEN
              HC1 = HIRC(A,2)
              HC2 = HIRC(B,1)
              HC3 = HIRC(A,1)
            ELSE
              IF(B == ICNT) EXIT BLOOP
              CYCLE BLOOP
            ENDIF
            ! is there a pair in the list for other combination?
            PC1 = HICLSTORIG(A)
            PC2 = HICLSTORIG(B)
            C = HC2
            D = HC3
            DO I = B+1,ICNT
              IF(HIRC(I,1) == C .AND. HIRC(I,2) == D .OR. &
                 HIRC(I,1) == D .AND. HIRC(I,2) == C) THEN ! 3some
                PC3 = HICLSTORIG(I)
                !omit lowest sensitivity
                CSSMINTMP = BIGDOUBLE
                SMALLEST = 0
                IF(CSSTMP(HC1) < CSSMINTMP) THEN
                  CSSMINTMP = CSSTMP(HC1)
                  SMALLEST = HC1
                ENDIF
                IF(CSSTMP(HC2) < CSSMINTMP) THEN
                  CSSMINTMP = CSSTMP(HC2)
                  SMALLEST = HC2
                ENDIF
                IF(CSSTMP(HC3) < CSSMINTMP) THEN
                  CSSMINTMP = CSSTMP(HC3)
                  SMALLEST = HC3
                ENDIF
                WRITE(IOUT,*)
                WRITE(IOUT,*)'    SET OF 3 INTERDEPENDENT PARAMETERS:'
                WRITE(IOUT,28) PNTMP(HC1),PNTMP(HC2),PNTMP(HC3)
                WRITE(IOUT,22) PC1,PC2,PC3
                WRITE(IOUT,29) PNTMP(SMALLEST),CSSMINTMP
                NPPCC = NPPCC + 1
                HIELIM(NPPCC) = SMALLEST
                DO J = 1,ICNT ! fill HICLST values for omitted parameters with zeros
                  DO K = 1,ICNT
                    IF(HIRC(J,1) == HIELIM(K) .OR. HIRC(J,2) == HIELIM(K)) &
                                                                   HICLST(J) = 0.D0
                 ENDDO
                ENDDO
                IF(NPPCC == NPNULL) EXIT ALOOP
                CYCLE BLOOP
              ENDIF
            ENDDO
          ENDDO BLOOP
        ...
        NPPCC3 = NPPCC
        ! FIND PAIRs with HIGHEST CORRELATIONs AND IDENTIFY LOWEST CSS PARAM !!
        IF(NPPCC < NPNULL) THEN
          WRITE(IOUT,250)
          WRITE(IOUT,34)SVDUPDATE_PCC
          ! SHORTEN HICLST and HIRC to omit parameters that have been eliminated
          ALLOCATE(HICLSTTMP(ICNT),HIRCTMP(ICNT,2))
          HICLSTTMP = 0.D0
          HIRCTMP = 0
          NEWCNT = 0
          DO I = 1,ICNT
            IF(HICLST(I) > 0.D0) THEN
              NEWCNT = NEWCNT + 1
              HICLSTTMP(NEWCNT) = HICLST(I)
              HIRCTMP(NEWCNT,1) = HIRC(I,1)
              HIRCTMP(NEWCNT,2) = HIRC(I,2)
            ENDIF
          ENDDO
          DEALLOCATE(HICLST,HIRC)
          ALLOCATE(HICLST(ICNT),HIRC(ICNT,2))
          HICLST = 0.D0
          HIRC = 0
          HICLST = HICLSTTMP
          HIRC = HIRCTMP
          DEALLOCATE(HICLSTTMP,HIRCTMP)
          CLOOP: DO I = 1,NEWCNT
            IF(HICLST(I) > 0.D0) THEN
              HC1 = HIRC(I,1)
              HC2 = HIRC(I,2)
              ...
              ENDIF
              WRITE(IOUT,*)
              WRITE(IOUT,*)'    SET OF 2 INTERDEPENDENT PARAMETERS:'
              WRITE(IOUT,28) PNTMP(HC1),PNTMP(HC2)
              WRITE(IOUT,22) HICLST(I)
              WRITE(IOUT,29) PNTMP(SMALLEST),CSSMINTMP
              NPPCC = NPPCC + 1
              HIELIM(NPPCC) = SMALLEST
              DO J = 1,ICNT ! fill HICLST values for omitted parameters with zeros
                ...
                ENDDO
              ENDDO
              IF(NPPCC == NPNULL) EXIT CLOOP
              CYCLE CLOOP
            ELSE
              EXIT CLOOP
            ENDIF
          ENDDO CLOOP
          IF(NPPCC == NPPCC3) WRITE(IOUT,35) SVDUPDATE_PCC
        ENDIF
        NPPCC2 = NPPCC - NPPCC3
      ENDIF
      ! FIND LOWEST CSS PARAMs !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      IF(NPPCC < NPNULL) THEN
        WRITE(IOUT,251)
        WRITE(IOUT,255)
        WRITE(IOUT,*)'    THE FOLLOWING LOW SENSITIVITY PARAMETERS WILL BE SET:'
        LOWSENS: DO I = 1,NPE
          AVAILABLE = .TRUE.
          ELIM: DO J = 1,NPE
            IF(PARNUMSORT(I) == HIELIM(J)) AVAILABLE = .FALSE.
          ENDDO ELIM
          IF(AVAILABLE) THEN
            WRITE(IOUT,*)
            WRITE(IOUT,256) PNSORT(I),CSSSORT(I)
            NPPCC = NPPCC + 1
            HIELIM(NPPCC) = PARNUMSORT(I)
            IF(NPPCC == NPNULL) EXIT LOWSENS
          ENDIF
        ENDDO LOWSENS
      ENDIF
      NPPCC1 = NPPCC - NPPCC3 - NPPCC2 - NPPCC0
    ENDIF
    IF(NPPCC > 0) THEN
      WRITE(IOUT,*)
      WRITE(IOUT,*)' PARAMETERS TO BE SET FOR OPTION2 SVD_UPDATE'
      WRITE(IOUT,*)' WERE DETERMINED BY CONSIDERING PARAMETERS WITH'
      WRITE(IOUT,*)'   FIRST: CSS RATIOS TO MAXIMUM < ', SVDUPDATE_CSSRAT
      WRITE(IOUT,*)'   THEN:  ABSOLUTE VALUES OF CSS < ', SVDUPDATE_CSSABS
      WRITE(IOUT,*)'   THEN:  THE PARAMETERS WITH LOWEST CSS OF SETS OF'
      WRITE(IOUT,*)'          3 INTERDEPENDENT PARAMETERS and then'
      WRITE(IOUT,*)'          PAIRS OF PARAMETERS'
      WRITE(IOUT,*)'   FINALLY: LOW CSS PARAMETERS INCLUDING:'
      WRITE(IOUT,*)
      WRITE(IOUT,*)
      WRITE(IOUT,*) &
      '      PARAMETER     HIGHEST-CORRELATION     SENSITIVITY     CSS RATIO'
      WRITE(IOUT,*) &
      '                     FOR THIS PARAMETER '
      WRITE(IOUT,*)
      IF(NPPCC0 > 0) THEN
        DO I = 1,NPPCC0
          WRITE(IOUT,26) PNORIG(HIELIM(I)),PCCMAXCOL(HIELIM(I)), &
                                CSSORIG(HIELIM(I)),CSSRAT(HIELIM(I))
        ENDDO
      ENDIF
      IF(NPPCC0 > 0 .AND. NPPCC0 < NPNULL) THEN
        IF(NPPCC0 > 0) THEN
          START = NPPCC0 + 1
        ELSE
          START = 1
        ENDIF
        OMITTED: DO I = START,(NPPCC0 + NPPCC2 + NPPCC3)
          HICOR: DO J = 1,NPEFAC
            IF(HICLSTORIG(J) == 0.D0) EXIT
            IF(HIELIM(I) == HIRCORIG(J,1) .OR. HIELIM(I) == HIRCORIG(J,2)) &
              THEN
              WRITE(IOUT,26) PNORIG(HIELIM(I)),HICLSTORIG(J), &
                                   CSSORIG(HIELIM(I)),CSSRAT(HIELIM(I))
              EXIT HICOR
            ENDIF
          ENDDO HICOR
        ENDDO OMITTED
        IF(NPPCC1 > 0) THEN
          IF((NPPCC0 + NPPCC2 + NPPCC3) > 0) THEN
            START = NPPCC0 + 1 + NPPCC2 + NPPCC3
          ELSE
            START = 1
          ENDIF
          DO I = START,NPPCC
      ...
      ENDIF
    ENDIF
    ! ADJUST FOR SET PARAMETERS
    ! IPTRSVDUPDATE will be 1 if parameter is to be updated and 0 if not
    IPTRSVDUPDATE = 1
    DO I=1,NPE
      IF(HIELIM(I) > 0) IPTRSVDUPDATE(HIELIM(I)) = 0
    ENDDO
    J = 0
    DO I=1,NPE
      IF(IPTRSVDUPDATE(I) == 0) THEN
    ...
    WRITE(IOUT,*)
    !
    DEALLOCATE(CSSTMP,PNTMP,CSSORIG,PNORIG,PCCMAT,PCCOPT2,PCCOPT2TMP, &
             PCCMAXCOL,PARNUMSORT)
    DEALLOCATE(HICLST,HIRC,HIELIM)
    DEALLOCATE(HICLSTORIG,HIRCORIG)
...
!===============================================================================
  SUBROUTINE UCODE_GEN_SVDCALCPAR &
                   (IOUT,ITERP,NPETMP,NPE,P_AT_MOMENT,SENSLOOP)
    IMPLICIT NONE
    !
    ...
    IF(.NOT. SENSLOOP) THEN
      ...
      ENDIF
      IF(ITERP > 0) THEN
        ! FINAL VALUES ESTABLISHED FROM SVD
        DO I=1,NPS
          PAREST(ITERP,I) = PVAL(I)
        ENDDO
      ENDIF
    ENDIF
    RETURN
...
!===============================================================================
  SUBROUTINE UCODE_GEN_SVDPOPX(MPR,NPETMP,SVDLOOP)
    ...
    ELSEIF(SVDLOOP == 3  .AND. MPR > 0) THEN
      DEALLOCATE(XPRI)
      ALLOCATE(XPRI(NPE,MPR))
      XPRI = XPRIHOLD
      DEALLOCATE(XPRIHOLD)
  ...
  !------------------------------------------------------------------
    ...
    IF(ICSS .GT. 0) THEN
      ...
      DO IP=1,NPENOW
        ...
        ENDIF
        CSS(IP)= DSQRT(CSS(IP)/(NOBS-IOMIT))
...
!===============================================================================
  ...
  !------------------------------------------------------------------
    ...
    ENDIF
    IOMITMODVAL = IOMIT
    IF((NOBS-IOMIT) < (NPE + 2) .AND. (OPTIMIZE .OR. OPTNLUNC)) &
      CALL UTL_STOP('TERMINATING, TOO MANY OMITTED OBSERVATIONS')
    RETURN
  END SUBROUTINE UCODE_UEV_OMIT
!===============================================================================
!===============================================================================
  SUBROUTINE UCODE_UEV_OMITSENS (IFAIL,FINALSTATS,IOUT,ITERP,KPI,SIMEQ1)
  !*****************************************************************************
  !     omitted preturbed obs and
  !     alter wtfull to have small weights
  !     and count those additional omitted obs
  !*****************************************************************************
  !        SPECIFICATIONS:
  !------------------------------------------------------------------
    IMPLICIT NONE
    ...
    DOUBLE PRECISION, DIMENSION(NOBS), INTENT(IN)    :: SIMEQ1 ! Simulated equivalents for perturbation of KPI
    !   Local variables
    INTEGER :: I, J, N, NNZ
    INTEGER(KIND=8) :: IIPOS
    LOGICAL :: PRTOMIT
    ! Formats
    100 FORMAT(/,' NUMBER OF OMITTED OBSERVATIONS = ',I6,/, &
        ' NUMBER OF OMITTED SENSITIVITIES DUE TO ',/, &
        '   PERTURBED SIMULATED EQUIVALENTS OF PARAMETER  :',1X,A,1X,'=', &
        1X,I6,/,'   DSS FOR PERTURBED SIMULATED EQUIVALENTS WITH OMITTED ', &
        'VALUES WILL BE ZERO')
    120 FORMAT(/,' NUMBER OF OMITTED PREDICTIONS = ',I6,/, &
        ' NUMBER OF OMITTED SENSITIVITIES DUE TO ',/, &
        '   PERTURBED SIMULATED EQUIVALENTS OF PARAMETER  :',1X,A,1X,'=', &
        1X,I6,/,'   DSS FOR PERTURBED SIMULATED EQUIVALENTS WITH OMITTED ', &
        'VALUES WILL BE ZERO')
    130 FORMAT(/,' NUMBER OF OMITTED OBSERVATIONS = ',I6,' OF ',I6,/, &
        ' NUMBER OF OMITTED SENSITIVITIES = ',I6,' OF ',I8,/,  &
        /,'   DSS FOR OMITTED SENSITIVITIES IS ZERO')
    140 FORMAT(/,' NUMBER OF OMITTED PREDICTIONS = ',I6,' OF ',I6,/, &
        ' NUMBER OF OMITTED SENSITIVITIES = ',I6,' OF ',I8,/,  &
        /,'   DSS FOR OMITTED SENSITIVITIES IS ZERO')
    ! Initialize
    AMESSAGE = ' '
    ERRSUB=' Error in subroutine UCODE_UEV_OMITSENS'
    IFAIL = 0
    ISENSOMIT = 0
    IF(KPI == 1) THEN
      ALLSENSOMIT = 0
    ...
    IF(OMIT_DEFAULT > 0) THEN
      DO I=1,NOBS
        DO J=1,OMIT_DEFAULT
          IF(SIMEQ1(I) == OMITIT(J)) THEN
            SENSOMIT(I) = -1
            ISENSOMIT = ISENSOMIT + 1
            ALLSENSOMIT = ALLSENSOMIT + 1
            PRTOMIT = .TRUE.
          ENDIF
        ENDDO
      ENDDO
      IF(.NOT. LINEARITY) THEN
        IF(PRTOMIT) THEN
          IF (IDEPTYPEU==1) THEN
            WRITE(IOUT,100)IOMIT,TRIM(PARNAM(IPTR(KPI))),ISENSOMIT
            IF(KPI == NPE) WRITE(IOUT,130)IOMIT,NOBS,ALLSENSOMIT,NOBS*NPE
          ELSEIF (IDEPTYPEU==2) THEN
            WRITE(IOUT,120)IOMIT,TRIM(PARNAM(IPTR(KPI))),ISENSOMIT
            IF(KPI == NPE) WRITE(IOUT,140)IOMIT,NOBS,ALLSENSOMIT,NOBS*NPE
          ENDIF
        ENDIF
      ENDIF
    ENDIF
    IF(KPI == NPE) THEN
      IF(((NPE*NOBS)-ALLSENSOMIT) < (NOBS -(NOBS*0.5))) THEN
        WRITE(IOUT,*)'TERMINATING, TOO MANY OMITTED SENSITIVITIES'
        WRITE(IOUT,*)'CHECK DEFAULT OMIT VALUES'
        WRITE(IOUT,*) &
        'INVESTIGATE WHETHER PROCESS MODEL HAS AN UNREASONABLE RESULT'
        WRITE(*,*)'TERMINATING, TOO MANY OMITTED SENSITIVITIES'
        WRITE(*,*)'CHECK DEFUALT OMIT VALUES'
        WRITE(*,*) &
        'INVESTIGATE WHETHER PROCESS MODEL HAS AN UNREASONABLE RESULT'
        CALL UTL_STOP('TERMINATING, TOO MANY OMITTED SENSITIVITIES')
      ENDIF
    ENDIF
    RETURN
  END SUBROUTINE UCODE_UEV_OMITSENS
!===============================================================================
!===============================================================================
  ...
  SUBROUTINE UCODE_EVA(IFAIL,IOUT,ITERP,NDEROBS,MAXITER,MPR,   &
    ...
    IF(IFO<3 .OR. STATS_ON_NONCONVERGE) THEN
      ...
      CALL REG_GNMOD_EVA_FINAL_OUT (IFAIL,IOUT,IFO,IND,IOMIT,3,ITERP, &
           ...
           RESIDSPRI,LNDETFP,LNDETFN,VAR,DTOFCORREL,CONDITION_NUMBER)
      IF(IFAIL > 0) THEN
        IF(SENS_NO_OPT) THEN
          IFAIL = 0
          CONTINUE
      ...
      IF(IFAIL < 1 .AND. (OPTIMIZE .OR. SENSITIVITIES) .AND. DATAEXCHANGE &
         .AND. ((NOBS - 2) >= NPE)) & ! added to accomodate nobs<npar Feb 2016
    ...
    IF(.NOT. CREATEINITFILES)THEN
      ...
      CALL UTLUCODE_DX_WRITE_DM(AIC,BIC,LNDETFN,HQ,IFO_IN,ITERP,KASHYAPNP, &
               ...
               DTLANP,DTLA,DTOFCORREL,CONDITION_NUMBER)
    ENDIF
    IF(SVDT .EQV. .FALSE.) THEN
      CALL UCODE_COPY_PRESVD(IOUT,OUTNAM,MPR)
    ENDIF
    IF(SVDT .EQV. .TRUE. .AND. SVDPHASEC == 'NEW' .AND. SVDLOOP == 1) THEN
      CALL UCODE_COPY_PRESVD(IOUT,OUTNAM,MPR)
    ENDIF
    IF(OPTIMIZE .EQV. .FALSE. .AND. SVDPHASEC == 'NEW' .AND. SVDLOOP == 2) THEN
      CALL UCODE_COPY_PRESVD(IOUT,OUTNAM,MPR)
    ENDIF
    IF(LINEARITY) THEN
      IF(LINEARITYADV) THEN
