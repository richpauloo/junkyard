MODULE REG_GN_UCODE
  ...
  PUBLIC &
    ...
    MAXITER, MAXCHANGE, MAXSTEP, MINSENRAT, NTYP, NUMINTERVALS, OMITIT, &
    OMIT_DEFAULT, OMIT_DEFAULT_SENS, OMIT_INSENS, OMIT_WEIGHT, &
    PAREST, PINC, PRNTA, &
    PRNTB, PRNTJ, &
    QNITER, QNSOSR, QUASINEWTON, REINCSENRAT, SCALING, STEPTYPE, SVD_PERTURB, &
    SVD_MAXCHANGE, SVD_TOLPAR, TOLPAR, TOLSOSC, WTADJFINAL, &
  ...
  DOUBLE PRECISION                               :: OMIT_DEFAULT_SENS
  DOUBLE PRECISION                               :: OMIT_WEIGHT
  ...
  DOUBLE PRECISION                               :: SVD_MAXCHANGE
  DOUBLE PRECISION                               :: SVD_PERTURB
  DOUBLE PRECISION                               :: SVD_TOLPAR
...
!    CHARACTER(LEN=3)                :: AWRP
    ...
    121 FORMAT(/,1X,'OMIT_WEIGHT = ',1PE15.6)
    ...
    150 FORMAT(/,1X,'ECHO MODIFIED GAUSS-NEWTON UCODE', &
                  ' REGRESSION CONTROL INPUT:',/,3X, '(SOME OF THESE ITEMS ',  &
                  'MAY BE CHANGED IN THE PARAMETER DATA BLOCKS)',/)
    ...
    IF (NGNUC>0) THEN
      ...
      GNUCINPUT: DO
        ...
        INFO: DO
          ...
          ELSEIF(PIPTR%KEYWORD == 'SVDTOLPAR') THEN
            READ(PIPTR%VALUE,*) SVD_TOLPAR
          ELSEIF(PIPTR%KEYWORD == 'SVDPERTURBAMT') THEN
            READ(PIPTR%VALUE,*) SVD_PERTURB
          ...
          ELSEIF (PIPTR%KEYWORD == 'TOLPARWTOSRENEW') THEN
            READ(PIPTR%VALUE,*) TMP1
            CALL UTL_CASE(TRIM(TMP1),TMP,1)
            IF (TMP == 'YES' .OR. TMP == 'Y' .OR.   &
              TMP == 'TRUE' .OR. TMP == 'T')   &
          ...
          ELSEIF(PIPTR%KEYWORD == 'OMITDEFAULTSENS') THEN
            READ(PIPTR%VALUE,*) OMIT_DEFAULT_SENS
          ELSEIF(PIPTR%KEYWORD == 'OMITWEIGHT') THEN
            READ(PIPTR%VALUE,*) OMIT_WEIGHT
          ...
          ELSEIF(PIPTR%KEYWORD == 'SVDMAXCHANGE') THEN
            READ(PIPTR%VALUE,*) SVD_MAXCHANGE
      ...
      IF (STDERRONE) THEN
        ...
        OPTIMIZE = .TRUE.
        TOLPAR = HUGE(TOLPAR)
        SVD_TOLPAR = HUGE(TOLPAR)
        MAXCHANGE = 1.D-20
        SVD_MAXCHANGE = 1.D-20
      ENDIF
    ELSE
      IF(OPTIMIZE) THEN
        IF(IVERB > 1) THEN
          ...
          WRITE(IOUT,100)'IF YOU WANT TO READ THIS BLOCK, CHECK BLOCK NAME SPELLING'
          WRITE(IOUT,100)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
          WRITE(IOUT,*)
        ENDIF
      ENDIF
    ...
    !   Write information (Gauss-Newton UCODE controls) to output file
      ...
      WRITE(IOUT,110) &
      'DEFAULT TolParWtOS                                       =',TOLWTTOS
      WRITE(IOUT,110) &
      'DEFAULT TolParWtOS×TolPar                                =', &
                                                             TOLWTTOS*TOLPAR
      WRITE(IOUT,110) &
      'DEFAULT WEIGHT UPDATING STOPS FOR MAXIMUM CHANGE         <', &
                                 (0.1*((TOLPAR * TOLWTTOS) - TOLPAR))+ TOLPAR
      WRITE(IOUT,100) &
      '  ABOVE VALUES ARE BASED ON DEFAULT TOLERANCES AND WILL'
      WRITE(IOUT,100) &
      '  DIFFER IF DIFFERENT VALUES ARE ASSIGNED FOR PARAMETER'
      WRITE(IOUT,100) &
      '  TOLERANCES IN THE PARAMETER DATA BLOCK'
      WRITE(IOUT,112) &
      'DEFAULT TolParWtOSRenew                                  =',TOLWTOSRENEW
      WRITE(IOUT,110) &
      ...
      ENDIF
      WRITE(IOUT,110) &
      'DEFAULT TOLERANCE ON % SVD PARAMETER CHANGE FOR CLOSURE  =',SVD_TOLPAR
      WRITE(IOUT,110) &
      'MAXIMUM FRACTIONAL SVD PARAMETER CHANGE                  =',SVD_MAXCHANGE
    ENDIF
    IF(OPTNLUNC) THEN
      LIMITS = 'BOTH'
    ...
    RETURN
    202 CALL UTL_STOP &
      ('END OF FILE REACHED on *.omit BEFORE "END OMIT_DATA" WAS READ')
    RETURN
    300 CALL UTL_STOP &
...
!======================================================================
  ...
  SUBROUTINE REG_GN_GEN (IFAIL,MPR,NOBS,NPE,NPS, &
                         ...
                         IOUT,IOMIT,IPTR,LN,CREATEINITFILES,OPTIM,OUTNAM, &
                         PADJ,PARNAM,PMAXCHANGE, &
                         PRIWTMAT,PTOLPAR,PVALINIT,PVALMAXC,PVALMINC, &
                         RESIDS,RESIDSPRI,RSQALL,STDERRONE, &
                         SVDLOOP,SVDT,UPDATE_FINAL_SENS,WTFULL,WTOSL, &
