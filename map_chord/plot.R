# ------------------------------------------------------------------------
# example: https://www.r-bloggers.com/2018/05/three-ways-of-visualizing-a-graph-on-a-map/
...
efp <- edges_for_plot %>% filter(weight >= 0.5)

ggplot(nodes) + country_shapes +
  geom_curve(aes(x = x, y = y, xend = xend, yend = yend,     # draw edges as arcs
                 color = weight, size = weight),
             data = efp, curvature = 0.33, alpha = 0.9,
             arrow = arrow(length = unit(efp$weight, "lines"), type = "closed")) +
...
p_edges <- ggplot(edges_for_plot) +
  geom_curve(aes(x = x, y = y, xend = xend, yend = yend,     # draw edges as arcs
                 color = weight, size = weight),
             curvature = 0.33, alpha = 0.7, 
             arrow = arrow(length = unit(edges_for_plot$weight, "lines"), type = "closed")) +
  scale_size_continuous(guide = FALSE, range = c(0.1, 2)) +  # scale for edge widths
